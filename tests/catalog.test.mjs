...
test("publishes only approved Fastly NGWAF request-feed, corp-activity, site-event, and site-activity metadata", () => {
  const item = custom.find((record) => record.slug === "fastly");
  assert.equal(item.name, "Fastly Account Audit and Next-Gen WAF Request Feed, Corp Activity, Site Security Events, and Site Activity");
  assert.equal(item.version, "0.5.0");
  assert.equal(item.status, "Experimental");
  ...
  assert.match(item.experimentalReason, /site-activity endpoint is \/analytics\/events, not the site security \/events endpoint/);
  assert.match(item.experimentalReason, /10,000-result query ceiling/);
  assert.match(item.experimentalReason, /rather than a current licensed tenant/);
  assert.match(item.experimentalReason, /CDN access logs and all WAF traffic are excluded/);
  assert.deepEqual(item.capabilities, ["Logs"]);
  assert.equal(item.fields.length, 69);
  ...
  assert.ok(item.fields.some((field) => field.field === "fastly.ngwaf_site_activity.raw" && field.type === "flattened"));
  assert.ok(item.fields.some((field) => field.field === "fastly.ngwaf_site_activity.site" && field.type === "keyword"));
